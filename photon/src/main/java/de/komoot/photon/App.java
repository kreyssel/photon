...
import de.komoot.photon.pbf.Extractor;
...
public class App {
	...
	public static void main(String[] rawArgs) {
		...
		if(args.isNominatimImport()) {
			...
			return;
		} else if (args.isPbfImport()) {
			startPbfImport(args, esServer, esClient);
			return;
		}
	 ...
	 * @param esNodeClient
	 */
	private static void startPbfImport(CommandLineArgs args, Server esServer, Client esNodeClient) {
		try {
			esServer.recreateIndex(); // dump previous data
		} catch (IOException e) {
			log.error("cannot setup index, elastic search config files not readable", e);
		...
		extractor.doImport(new File("src/test/resources/berlin-latest.osm.pbf"), importer);
